...
def checar_qt_letras(lista,index):

    def teste(x):
        return [x,lista.count(x)]
    index = teste
    return index
...
def cod_run_lenght(string):
    ...
    qt_letra = map(checar_qt_letras(resultado,0),list(set(resultado_fil)))
...
def main():
    print(f'Decodificada = {cod_run_lenght("AAAAAABBA")}')
